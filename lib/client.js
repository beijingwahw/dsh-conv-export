window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function buildMarkdown(title, messages, exportedAt) {
			const parts = [];
...
`;
		...
		*/
		function buildExportHtml(title, messages, exportedAt) {
			...
			return `<div class="x-wrap"><h1 class="x-title">${esc(title)}</h1><p class="x-meta">${esc(t("meta.exported"))} ${esc(exportedAt)}</p>${turns}</div>`;
		}
		/**
		...
		* @param exportedAt - ISO timestamp line.
		*/
		function exportPdf(title, messages, exportedAt) {
			const win = window.open("", "_blank");
			if (win === null) return;
			win.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${title.replace(/</g, "&lt;")}</title><style>${EXPORT_CSS}</style></head><body>${buildExportHtml(title, messages, exportedAt)}</body></html>`);
			win.document.close();
			win.focus();
		...
		*/
		async function exportImage(title, messages, exportedAt) {
		...
		var ExportController = class {
			...
			*/
			async run(kind) {
				...
				const exportedAt = (/* @__PURE__ */ new Date()).toISOString().replace("T", " ").slice(0, 19);
				this.busy = true;
				try {
					if (kind === "markdown") downloadBlob(`${stem}.md`, "text/markdown;charset=utf-8", buildMarkdown(title, messages, exportedAt));
					else if (kind === "pdf") exportPdf(title, messages, exportedAt);
					else await exportImage(title, messages, exportedAt);
				} catch {
